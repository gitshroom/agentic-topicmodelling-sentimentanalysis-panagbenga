...
# Multilingual preprocessing v2 (stricter cleaning, blocked-account filter).
# =========================
...
def main():
    ...
    print("[preprocessing_second] Loading dataset...")
    ...
    print(f"[preprocessing_second] Blocked account filter removed {before - len(df)} rows")

    df["year"] = df["timestamp"].apply(extract_year)

    df = df.dropna(subset=["year"])

    print("[preprocessing_second] Cleaning text...")
    ...
    df.to_csv(config.PREPROCESSED_FILE_SECOND, index=False)

    print(f"[preprocessing_second] Saved {len(df)} rows -> {config.PREPROCESSED_FILE_SECOND}")
