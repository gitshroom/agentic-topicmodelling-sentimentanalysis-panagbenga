...
#
# Data flow:
#   topic_modelling  →  TOPIC_OUTPUT_FILE   (year → clusters → topics)
#   sentiment_analysis reads TOPIC_OUTPUT_FILE
#                    →  SENTIMENT_OUTPUT_FILE (year → clusters → topics + sentiment)
...
def main():
    log_banner(logger, "Orchestrator — per-year → per-cluster → per-topic pipeline")
    ...
    log_banner(logger, "Step B: Sentiment Analysis (per topic per cluster per year)")
