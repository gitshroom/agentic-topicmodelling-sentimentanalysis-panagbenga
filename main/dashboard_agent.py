"""
dashboard_agent.py
Top-10 Topics per Year + Plain-Language Explainer + Timeline tabs
...
.summary-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:.8rem;margin-bottom:2rem}
...
function boot(d){
  ...
  document.getElementById('summary').innerHTML=
    [{l:'Years',v:s.total_years||0},{l:'Posts',v:(s.total_docs||0).toLocaleString()},{l:'Topics',v:s.total_topics||0}]
  ...
  if(years.length) selTop10(years[years.length-1]);
}
...
function getTop10(yearEntry){
  const all=(yearEntry?.topics||[]).slice();
